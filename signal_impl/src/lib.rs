...
pub enum HandlingSignal {
    ...
    UserSignal(TrapContext), // 是用户信号，需要保存之前的用户栈
}
...
impl Signal for SignalImpl {
    ...
    fn handle_signals(&mut self, current_context: &mut TrapContext) -> SignalResult {
            ...
            match signal {
                ...
                _ => {
                    if let Some(action) = self.actions[signal as usize] {
                        ...
                        // 保存原来用户程序的上下文信息
                        self.handling = Some(HandlingSignal::UserSignal(*current_context));
                        // 修改返回后的 pc 值为 handler，修改 a0 为信号编号
                        //println!("handle pre {:x}, after {:x}", current_context.sepc, action.handler);
                        current_context.sepc = action.handler;
                        current_context.x[10] = signal as usize;
                        SignalResult::Handled
                    } else {
    ...
    fn sig_return(&mut self, current_context: &mut TrapContext) -> bool {
        match self.handling {
            Some(HandlingSignal::UserSignal(old_ctx)) => {
                //println!("return to {:x} a0 {}", old_ctx.sepc, old_ctx.x[10]);
